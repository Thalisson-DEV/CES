package com.sipel.CES.obras.service;
...
import com.sipel.CES.obras.repositories.ObraRepository;
import com.sipel.CES.obras.repositories.StatusObraRepository;
        ...
        try (InputStream inputStream = file.getInputStream()) {
            ...
            while (rows.hasNext()) {
                ...
                try {
                    ...
                    if (baseObraNome != null && !baseObraNome.trim().isEmpty() && !"NA".equalsIgnoreCase(baseObraNome.trim())) {
                        baseObra = BaseOperacionalRepository.findByNomeBase(baseObraNome)
                                .orElseThrow(() -> new RuntimeException("Base de Obra '" + baseObraNome + "' inválida."));
                    }

                    BaseOperacional baseSaque = null;
                    if (baseSaqueNome != null && !baseSaqueNome.trim().isEmpty() && !"NA".equalsIgnoreCase(baseSaqueNome.trim())) {
                        baseSaque = BaseOperacionalRepository.findByNomeBase(baseSaqueNome)
                    ...
                    novaObra.setBaseObra(baseObra); // Pode ser null se for 'NA'
